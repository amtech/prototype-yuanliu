...
function onIconLink(icon: HTMLElement, blue: IBlue, prop: string, type: "method" | "event" | "property", blues: IBlue[] = getCurPage().blues) {
    ...
    icon.ondragstart = (e: any) => {
        ...
        e.dataTransfer.setData("data-blue", JSON.stringify({ blue: blue, point: prop, type: type, x: getX(getDivClientLeft(e.target)), y: getY(getDivClientTop(e.target)) }));
    ...
    icon.ondrop = (e: any) => {
        ...
        var curve = paints.bezierCurve("fill:none;stroke:" + linkColor + ";stroke-width:2;", x, y, getX(getDivClientLeft(e.target)), getY(getDivClientTop(e.target)));
        ...
        var link: IBlueLink = {
            ...
            position: {
                x0: x, y0: y, x1: getX(getDivClientLeft(e.target)), y1: getY(getDivClientTop(e.target))
            }
        };
